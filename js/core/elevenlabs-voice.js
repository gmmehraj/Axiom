...
(function (global) {
  ...
  async function authHeaders() {
    ...
    return { Authorization: 'Bearer ' + session.access_token, apikey: (typeof SUPABASE_ANON_KEY !== 'undefined' ? SUPABASE_ANON_KEY : ''), 'Content-Type': 'application/json' };
  }

  async function requestAudio(payload) {
    var url = getSupabaseUrl();
    if (!url) throw new Error('Axiom voice service is not configured.');
    var response = await fetch(url.replace(/\/$/, '') + '/functions/v1/' + EDGE_ACTION, { method: 'POST', headers: await authHeaders(), body: JSON.stringify(payload) });
    if (!response.ok) {
      var detail = '';
  ...
  function playBlob(blob, opts) {
    return new Promise(function (resolve, reject) {
      var url = URL.createObjectURL(blob), audio = new Audio(url), cleaned = false;
  ...
  var adapter = {
    ...
    speak: async function (text, opts) {
      ...
      if (global.__AxiomElevenAudio) { try { global.__AxiomElevenAudio.pause(); global.__AxiomElevenAudio.currentTime = 0; } catch (_) {} }
      var blob = await requestAudio({
        action: 'speak', text: String(text || ''), voice_id: voiceId, model_id: modelId,
        language_code: String(lang).replace('_', '-'),
        voice_settings: {
    ...
    cancel: function () { if (global.__AxiomElevenAudio) { try { global.__AxiomElevenAudio.pause(); global.__AxiomElevenAudio.currentTime = 0; } catch (_) {} } }
  };
