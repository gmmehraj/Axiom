...
(function (global) {
  ...
  function valueOrUnavailable(value, fallback) { return value === null || value === undefined || value === '' ? (fallback || 'Unavailable') : String(value); }
  ...
  function init() {
    ...
    function getSnapshot() {
      ...
      return { brain: state, activity: canonical || state.activity || 'idle', memory: memoryStats };
    }
    ...
    function render() {
      ...
      const automation = b.automation && b.automation.status !== 'idle' ? `${b.automation.status}${b.automation.workflowName ? ' · ' + b.automation.workflowName : ''}` : 'Idle / unavailable';
      ...
      const views = {
        state: { title: 'Live AI State', description: 'Only state emitted by Axiom\'s real AI state systems is shown here.', cards: [card('Activity', s.activity, 'AxiomAIState / AxiomBrain'), card('Last interaction', formatTime(b.lastInteraction), 'Persisted Brain timestamp'), card('Day', valueOrUnavailable(b.day), 'Derived from first launch')] },
        model: { title: 'Active Model', description: 'The model shown is the model reported by the existing model-selection state. Unknown stays unknown.', cards: [card('Current model', model, 'No hardcoded provider/model'), card('Model state', b.activeModel ? 'Available' : 'Unavailable', 'Source: AxiomBrain.activeModel')] },
        tools: { title: 'Tool Activity', description: 'Capability state comes from the existing agent event stream.', cards: [card('Tool status', tool, b.toolActive ? 'Live capability call' : 'No active capability'), card('Automation', automation, 'Existing automation bridge state')] },
        conversation: { title: 'Conversation State', description: 'The Brain follows the conversation lifecycle instead of inventing reasoning telemetry.', cards: [card('Conversation', conversation, b.activeConversationId ? 'Active conversation id' : 'No active conversation'), card('Activity', s.activity, 'Canonical live state')] },
        memory: { title: 'Memory State', description: 'Memory numbers come from the same persisted Memory Engine used by the Memory page.', cards: s.memory ? [card('Memory items', s.memory.memoryCount, 'AxiomMemoryEngine.getStats()'), card('Pinned', s.memory.pinnedCount, 'Persisted pinned memories'), card('Conversations', s.memory.conversationCount, 'Memory Engine conversation records'), card('Working memory', `${Math.round(s.memory.shortTermCacheLoad * 100)}%`, 'Derived from real working-memory items')] : [card('Memory Engine', 'Unavailable', 'No Memory Engine loaded on this page')] },
        system: { title: 'Connected Systems', description: 'No synthetic health or performance score is displayed.', cards: [card('AI state source', aiState ? 'AxiomAIState' : 'AxiomBrain', 'Canonical activity source'), card('Brain source', brain ? 'AxiomBrain' : 'Unavailable', 'Persistent cross-page state'), card('Automation', automation, 'Real workflow state when available'), card('Telemetry', 'Only verified signals', 'Latency, token speed and confidence are not fabricated')] }
      };
      const view = views[selected] || views.state;
      content.innerHTML = `<div class="brain-section active"><div class="brain-section-head"><div><h2>${escapeHtml(view.title)}</h2><p>${escapeHtml(view.description)}</p></div></div><div class="brain-metrics-grid">${view.cards.join('')}</div></div>`;
      nav.querySelectorAll('[data-truth-section]').forEach(btn => btn.classList.toggle('active', btn.dataset.truthSection === selected));
    }
    nav.querySelectorAll('[data-truth-section]').forEach(btn => btn.addEventListener('click', () => { selected = btn.dataset.truthSection || 'state'; render(); }));
