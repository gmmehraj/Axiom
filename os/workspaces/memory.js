...
function axiomEnsureMemoryEngine(done) {
  if (window.AxiomMemoryEngine) { done(window.AxiomMemoryEngine); return; }
  const script = document.createElement('script');
  script.src = 'os/core/memory-engine.js';
  script.onload = () => done(window.AxiomMemoryEngine || null);
...
window.AxiomWorkspaces.memory = {
  ...
  render: function (container) {
    axiomEnsureMemoryEngine((engine) => {
      if (!engine) {
        container.innerHTML = `
          <div class="ax-workspace-window" data-motion="fade-in">
            ...
            <div class="ax-workspace-window-body"><div class="ax-workspace-fallback"><h2>Memory unavailable</h2><p>The real Memory Engine could not be loaded.</p></div></div>
          </div>`;
        return;
      }
          ...
          <div class="ax-workspace-window-body">
            <div class="ax-memory-page" style="max-width:1100px;margin:0 auto;padding:clamp(12px,2vw,28px);">
              <div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap;margin-bottom:20px;">
                <div><h2 style="margin:0 0 6px;">Memory System</h2><p style="margin:0;color:var(--ax-text-3);max-width:650px;">Persistent memory from the same engine used by Axiom's Memory page.</p></div>
      ...
      function render() {
        ...
        if (!memories.length) {
          listEl.innerHTML = `<div class="ax-workspace-fallback" style="padding:48px 20px;"><h3>${search || filter !== 'all' ? 'No matching memories' : 'No memories yet'}</h3><p>${search || filter !== 'all' ? 'Try a different search or filter.' : 'Axiom will build memory as you interact with it.'}</p></div>`;
          return;
        }
        listEl.innerHTML = `<div style="overflow:auto;background:var(--ax-glass);border:1px solid var(--ax-border);border-radius:var(--ax-radius-sm);"><table style="width:100%;min-width:620px;border-collapse:collapse;font-size:.82rem;"><thead><tr style="border-bottom:1px solid var(--ax-border);"><th style="padding:10px 14px;text-align:left;color:var(--ax-text-3);font-size:.68rem;text-transform:uppercase;">Memory</th><th style="padding:10px 14px;text-align:left;color:var(--ax-text-3);font-size:.68rem;text-transform:uppercase;">Agent</th><th style="padding:10px 14px;text-align:left;color:var(--ax-text-3);font-size:.68rem;text-transform:uppercase;">Updated</th><th style="padding:10px 14px;text-align:left;color:var(--ax-text-3);font-size:.68rem;text-transform:uppercase;">Confidence</th><th style="padding:10px 14px;text-align:left;color:var(--ax-text-3);font-size:.68rem;text-transform:uppercase;">Tags</th></tr></thead><tbody>${memories.map(m => `<tr style="border-bottom:1px solid rgba(255,255,255,.04);"><td style="padding:12px 14px;color:var(--ax-text-2);max-width:520px;">${escapeHtml(m.text)}</td><td style="padding:12px 14px;color:var(--ax-text-2);white-space:nowrap;">${escapeHtml(m.agent || 'Unknown')}</td><td style="padding:12px 14px;color:var(--ax-text-3);white-space:nowrap;">${relativeDate(m.updatedAt || m.createdAt)}</td><td style="padding:12px 14px;color:var(--ax-text-2);white-space:nowrap;">${typeof m.confidence === 'number' ? Math.round(m.confidence * 100) + '%' : 'Unavailable'}</td><td style="padding:12px 14px;color:var(--ax-text-3);">${escapeHtml((m.tags || []).join(', ')) || '—'}</td></tr>`).join('')}</tbody></table></div>`;
      }
      ...
      const observer = new MutationObserver(() => {
        if (!document.body.contains(container) && unsubscribe) {
          unsubscribe(); unsubscribe = null; observer.disconnect();
        }
      });
